...
import { getNewsImage } from '@/lib/images'
...
export function NewsCard({ title, imageSrc, imageAlt, href }: NewsCardProps) {
  const image = getNewsImage(imageSrc)
  ...
  return (
    <article className="flex w-full flex-col gap-4 lg:relative lg:aspect-[800/500] lg:h-news-card lg:w-news-card lg:max-w-none lg:overflow-hidden lg:rounded-tl-card lg:rounded-tr-card lg:rounded-br-card lg:rounded-bl-none">
      ...
      <div className="flex flex-col items-start gap-3 lg:hidden" data-testid="news-card-mobile">
        <h3 className="font-general text-news-card-mobile font-medium leading-[1.3] tracking-normal text-neutral-950">
          {title}
        </h3>
      ...
      <a
        ...
        aria-label={`Read article: ${title}`}
      >
        <h3 className="font-general text-news-overlay font-medium leading-[1.3] tracking-normal text-neutral-950">
          {title}
        </h3>
        <span className="mt-2 inline-block border-b border-ink pb-0.5 font-general text-caption font-medium leading-[1.3] tracking-[0.5px] text-ink uppercase">
          Read article
        </span>
